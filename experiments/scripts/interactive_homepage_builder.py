#!/usr/bin/env python3
"""
Interactive Homepage Builder - E-commerce Homepage Generator

Specialized script that uses the Homepage Builder agent to create complete,
working homepages for e-commerce stores.

Key Features:
- Uses Homepage Builder agent (focused on homepage only)
- Faster generation (10-15 minutes vs 25-40 minutes for full store)
- Same accuracy and quality as Architect V2
- All CLI features from interactive_agent_v2.py

Usage:
    # Interactive mode (Homepage Builder)
    python interactive_homepage_builder.py

    # Single command mode
    python interactive_homepage_builder.py --message "Create a luxury jewelry homepage"

    # With specific chat ID
    python interactive_homepage_builder.py --chat-id my_homepage_123

    # As a module
    from experiments.scripts.interactive_homepage_builder import run_homepage_builder_streaming
    async for chunk in run_homepage_builder_streaming(chat_id, message):
        print(chunk)
"""
...
def copy_scaffold_infrastructure(chat_root: str) -> bool:
    ...
    This copies comprehensive e-commerce infrastructure:
    - Core: router.js, utils.js, base-component.js
    - E-commerce: cart.js, wishlist.js, product-filters.js
    - Tools: analytics.js, validators.js

    The homepage builder agent will create homepage-specific components.
    ...
    # It should be in the project root (parent of experiments)
    script_dir = Path(__file__).parent.parent.parent  # Go up to project root
    infra_source = script_dir / "scaffold_infrastructure"

    if not infra_source.exists():
        # Try alternate location in experiments folder
    ...
    try:
        ...
        print(f"   (router, utils, cart, wishlist, filters, analytics, validators, base-component)", file=sys.stderr)
        return True
...
async def run_homepage_builder_streaming(
    chat_id: str,
    user_message: str,
    ...
    """
    Run the Homepage Builder agent with streaming responses.

    This version creates only the homepage and related components.
    ...
    Args:
        ...
        max_turns: Maximum turns for agent execution (default 300)
        on_event: Optional callback for event processing
        init_scaffold: If True, automatically copy scaffold template to project
        debug: If True, write planning data to disk (.doxii/features.json, tasks.json)
    ...
    Example:
        async for chunk in run_homepage_builder_streaming("my_chat", "Create a homepage"):
    ...
    if chat_root is None:
        # Create in experiments/test_output_homepage for easy access
        base_dir = Path(__file__).parent.parent / "test_output_homepage"
    ...
    if init_scaffold:
        try:
            ...
            if infra_copied:
                ...
                print(f"   Homepage Builder will create homepage components\n", file=sys.stderr)
        except Exception as e:
    ...
    # Create Homepage Builder agent
    agent = create_homepage_builder_agent()
    print(f"🏗️  Running HOMEPAGE BUILDER\n", file=sys.stderr)
    print(f"   Creates complete, working homepages\n", file=sys.stderr)
    ...
    try:
        ...
        # Stream events
        current_agent = "Homepage_Builder"
...
def _check_project_initialized(chat_root: str) -> bool:
    ...
    # A project is initialized when the agent has generated index.html
...
class InteractiveHomepageBuilder:
    """Interactive CLI for the Homepage Builder agent."""

    def __init__(self, chat_id: str = None, cms_base_url: str = "https://api.example.com", debug: bool = False):
        self.chat_id = chat_id or f"homepage_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def print_header(self):
        ...
        print("=" * 80)
        print("DOXII HOMEPAGE BUILDER - E-commerce Homepage Generator")
        print("=" * 80)
        print()
        print(f"Chat ID: {self.chat_id}")
        print(f"Project Root: {os.path.abspath(chat_root)}")
        print()
        print("Mode: HOMEPAGE BUILDER (Homepage Only)")
        print("  • Creates complete, working homepages")
        print("  • Hero section with animations")
        print("  • Featured products showcase")
        print("  • Categories grid")
        print("  • Newsletter signup")
        print("  • Trust badges and social proof")
        print("  • Infrastructure auto-initialized")
        print("  • Expected time: 10-15 minutes")
        print()
        if self.debug:
            print("⚠️  DEBUG MODE ENABLED")
            print("  • Planning data will be written to .doxii/ directory")
            print("  • Features saved to .doxii/features.json")
    ...
    async def handle_command(self, command: str) -> bool:
        ...
        if cmd == "/help":
            ...
            if os.path.exists(chat_root):
                ...
                print(f"  Agent: Homepage Builder")
                print()
            else:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="DOXII Homepage Builder - E-commerce Homepage Generator",
...
Examples:
  ...
  # Specify chat ID
  python interactive_homepage_builder.py --chat-id my_homepage

  # Specify CMS URL
  python interactive_homepage_builder.py --cms-url https://cms.example.com/api
        """
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode (writes planning data to .doxii/ directory)"
    )
    ...
    agent = InteractiveHomepageBuilder(
    ...
    # Single message mode
    if args.message:
        print("DOXII Homepage Builder - Single Message Mode")
...
async def example_usage():
    ...
    """
    chat_id = "example_homepage_integration"
    user_message = "Create a luxury jewelry homepage for 'Aurelia Gems'"

    print("Example: Using run_homepage_builder_streaming() as a module")
    print("=" * 60)
    print()
