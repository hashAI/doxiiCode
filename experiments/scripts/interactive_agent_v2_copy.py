#!/usr/bin/env python3
"""
Interactive Multi-Agent System V2 - Component Library-Powered

Enhanced version that uses Architect V2 with component library integration.

Key Improvements:
- Uses Architect V2 with access to 43+ pre-built components
- Faster store generation with fewer bugs
- Maintains same effectiveness and features
...
def copy_scaffold_infrastructure(chat_root: str) -> bool:
    ...
    The architect V2 agent will use the component library for UI components.
...
async def run_agent_streaming(
    ...
    This version uses Architect V2 with component library integration.
    ...
    if init_scaffold:
        try:
            ...
            if infra_copied:
                ...
                print(f"   Architect V2 will use component library for UI components\n", file=sys.stderr)
        except Exception as e:
    ...
    # Create agent - Architect V2 or fallback to orchestrator
    if use_architect_v2:
        agent = create_architect_v2_agent()
        print(f"🏗️  Running ARCHITECT V2 (Component Library-Powered)\n", file=sys.stderr)
        print(f"   Access to 43+ pre-built components\n", file=sys.stderr)
    else:
    ...
    try:
        ...
        # Stream events
        current_agent = "Architect_V2" if use_architect_v2 else "Orchestrator"
...
class InteractiveAgentV2:
    ...
    def __init__(self, chat_id: str = None, cms_base_url: str = "https://api.example.com", use_architect_v2: bool = True, debug: bool = False):
    ...
    def print_header(self):
        ...
        print("=" * 80)
        if self.use_architect_v2:
            print("DOXII ARCHITECT V2 - Component Library-Powered")
        else:
        ...
        print()
        if self.use_architect_v2:
            print("Mode: ARCHITECT V2 (Component Library-Powered)")
            print("  • Access to 43+ pre-built, tested components")
            print("  • Categories: Headers, Footers, Heroes, Product Cards, Grids,")
            print("                Galleries, Carts, Filters, Navigation, and more")
            print("  • Faster development with fewer bugs")
            print("  • Infrastructure auto-initialized (router, utils, cart, etc.)")
            print("  • Smart component recommendations for your store type")
            print("  • Unique stores through intelligent component selection")
        else:
            print("Mode: ORCHESTRATOR (automatic routing)")
            print("  • Architect V2 - Creates new projects with component library")
    ...
    async def handle_command(self, command: str) -> bool:
        ...
        if cmd == "/help":
            ...
            if os.path.exists(chat_root):
                ...
                print(f"  Agent Version: Architect V2 (Component Library)")
                print()
            else:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="DOXII Multi-Agent System V2 - Component Library-Powered",
...
Examples:
  ...
  python interactive_agent_v2.py --cms-url https://cms.example.com/api
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Run architect V2 agent (default, component library-powered)"
    )
    ...
    # Single message mode
    if args.message:
        print("DOXII Architect V2 - Single Message Mode (Component Library-Powered)")
