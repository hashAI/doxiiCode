#!/usr/bin/env python3
"""
Interactive Multi-Agent System - Production-Ready Script

This script provides a comprehensive interface to the DOXII multi-agent system with:
- Interactive CLI for user input
- Real-time streaming responses
- Automatic agent routing via Orchestrator
- File operation tracking
- Error handling and recovery
- Metrics collection

Can be used:
1. Standalone for testing and demos
2. As a module imported by the main project
3. As a reference implementation for production integration
...
Usage:
    ...
    # Single command mode
    python interactive_agent.py --message "Create a bookstore website"
    
    # With specific chat ID
    python interactive_agent.py --chat-id my_project_123
    
    # As a module
    from experiments.scripts.interactive_agent import run_agent_streaming
...
def copy_scaffold_infrastructure(chat_root: str) -> bool:
    ...
    The architect agent will generate all business-specific files.
...
async def run_agent_streaming(
    ...
    """
    Run the multi-agent system with streaming responses.

    This is the main function that can be imported and used in production.
    ...
    Args:
        ...
        use_architect: If True, run architect agent directly (bypasses orchestrator)
    ...
    if init_scaffold:
        try:
            ...
            if infra_copied:
                ...
                print(f"   Agent will generate business-specific files\n", file=sys.stderr)
        except Exception as e:
    ...
    # Create agent - either architect directly or orchestrator
    if use_architect:
        agent = create_architect_agent()
        print(f"🏗️  Running ARCHITECT agent directly\n", file=sys.stderr)
    else:
    ...
    try:
        ...
        # Stream events
        current_agent = "Architect" if use_architect else "Orchestrator"
...
class InteractiveAgent:
    ...
    def __init__(self, chat_id: str = None, cms_base_url: str = "https://api.example.com", use_architect: bool = False):
        self.chat_id = chat_id or f"interactive_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def print_header(self):
        ...
        if self.use_architect:
            print("DOXII ARCHITECT AGENT - Direct Testing Mode")
        else:
            print("DOXII MULTI-AGENT SYSTEM - Interactive Mode")
        print("=" * 80)
        print()
        ...
        print()
        if self.use_architect:
            print("Mode: ARCHITECT DIRECT (bypasses orchestrator)")
            print("  • Comprehensive infrastructure auto-initialized")
            print("    (router, utils, cart, wishlist, filters, analytics, validators, base-component)")
            print("  • Architect generates custom files based on your requirements")
            print("  • Each store will be unique with custom layouts and components")
            print("  • Ideal for testing ecommerce store generation")
        else:
            print("Mode: ORCHESTRATOR (automatic routing)")
            print("  • Architect - Creates new projects from scratch")
    ...
    async def process_message(self, message: str):
        ...
        async for chunk in run_agent_streaming(
            ...
            use_architect=self.use_architect
        ):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="DOXII Multi-Agent System - Interactive Interface",
...
Examples:
  ...
  # Test architect agent directly (generates unique store)
  python interactive_agent.py --architect --message "Create a luxury jewelry store"

  # Interactive architect mode
  python interactive_agent.py --architect

  # Single message mode
  python interactive_agent.py --message "Create a bookstore website"

  # Specify chat ID
  python interactive_agent.py --chat-id my_project

  # Specify CMS URL
  python interactive_agent.py --cms-url https://cms.example.com/api
        """
    )
    ...
    parser.add_argument(
        "--architect",
        action="store_true",
        help="Run architect agent directly (bypasses orchestrator, generates unique custom stores)"
    )
    ...
    agent = InteractiveAgent(
        ...
        use_architect=args.architect
    )

    # Single message mode
    if args.message:
        if args.architect:
            print("DOXII Architect Agent - Single Message Mode")
        else:
            print("DOXII Multi-Agent System - Single Message Mode")
        print(f"Chat ID: {agent.chat_id}")
        print(f"Mode: {'ARCHITECT DIRECT' if args.architect else 'ORCHESTRATOR'}")
        print()
...
async def example_usage():
    ...
    """
    chat_id = "example_integration"
    user_message = "Create a simple bookstore website"
    
    print("Example: Using run_agent_streaming() as a module")
    print("=" * 60)
    print()
